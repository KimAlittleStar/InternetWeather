/**************************************************************************
* 功能函数说明此LCD 驱动函数 参考 于 正点原子 LCD 驱动函数 
* 文件来源：库函数版本
* 
***************************************************************************/
...
{  							  
	...
	temp = (size%24==0)?(24):((size%16==0)?(16):(12));
	csize = (temp/8+((temp%8)?1:0))*(temp/2);
	temp = 0;	
	...
	{   
		...
		for(t1=0;t1<8;t1++)
		{	
			for(t2 = 0;t2<pointsize;t2++)
			{
				for(t3 = 0;t3<pointsize;t3++)
				{
					if(temp&0x80)LCD_Fast_DrawPoint(x+t2,y+t3,POINT_COLOR);
					else if(mode==0)LCD_Fast_DrawPoint(x+t2,y+t3,BACK_COLOR);
				}
			}
